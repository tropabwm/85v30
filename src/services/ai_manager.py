...
class AIManager:
    ...
    def __init__(self):
        ...
        if not self.providers and not self.offline_mode:
            logger.warning("⚠️ Nenhum provedor de IA disponível, ativando modo offline")
            self.offline_mode = True

        if self.offline_mode:
            logger.info("🔧 AI Manager em modo offline - usando fallbacks locais")
        else:
    ...
    def generate_analysis(self, prompt: str, max_tokens: int = 8192, provider: Optional[str] = None) -> Optional[str]:
        ...
        if not provider_name and not self.offline_mode:
            logger.warning("⚠️ Nenhum provedor de IA disponível, tentando modo offline")
            self.offline_mode = True

        if self.offline_mode:
            logger.info("🔧 Usando modo offline para geração de conteúdo.")
            return self._generate_offline_content(prompt)

        if not provider_name:
            raise Exception("❌ NENHUM PROVEDOR DE IA DISPONÍVEL E MODO OFFLINE DESATIVADO.")
    ...
    def _generate_with_gemini(self, prompt: str, max_tokens: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"❌ Erro em _generate_with_gemini: {e}")
            raise e
    ...
    def _generate_with_groq(self, prompt: str, max_tokens: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"❌ Erro em _generate_with_groq: {e}")
            raise e
    ...
    def _generate_with_openai(self, prompt: str, max_tokens: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"❌ Erro em _generate_with_openai: {e}")
            raise e
    ...
    def _try_fallback(self, prompt: str, max_tokens: int, exclude: List[str]) -> Optional[str]:
        ...
        if not available_providers:
            ...
            # Se chegou aqui, todos falharam - usa fallback offline
            logger.warning("⚠️ Todos os provedores falharam, usando geração offline")
            return self._generate_offline_content(prompt)
    ...
    def _generate_offline_content(self, prompt: str) -> str:
        """Gera conteúdo offline como fallback final."""
        # Esta é uma implementação placeholder. Em um cenário real,
        # você carregaria um modelo local ou usaria um mecanismo de resposta pré-definido.
        logger.warning("🧠 Gerando conteúdo offline (placeholder)")
        return f"Conteúdo offline para: {prompt[:100]}..."
