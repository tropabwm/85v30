...
class UltraDetailedAnalysisEngine:
    ...
    def _execute_massive_real_research_original(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        queries = self._gerar_queries_inteligentes(data.get('segmento', ''), data.get('produto', ''))
...
"""
    ...
    def _contains_simulated_data(self, analysis: Dict[str, Any]) -> bool:
        ...
        # Palavras que indicam simulação (removido 'n/a' para ser menos rigoroso)
        simulation_indicators = [
            'dados simulados', 'exemplo fictício', 'placeholder', 'mock data'
        ]
        ...
        required_sections = ['avatar_ultra_detalhado']  # Apenas avatar é obrigatório
        ...
        if len(insights) < 1:  # Reduzido para 1
            logger.warning(f"⚠️ Poucos insights: {len(insights)} - mas continuando")
            # Não considera simulação mais

        # Verifica qualidade dos insights
        substantial_insights = [insight for insight in insights if len(insight) > 50]
        if len(substantial_insights) < len(insights) * 0.3:  # Reduzido para 30%
            logger.warning(f"⚠️ Alguns insights superficiais: {len(substantial_insights)}/{len(insights)} - mas aceitável")
    ...
    def _gerar_queries_inteligentes(self, segmento: str, produto: str = "") -> List[str]:
        """Gera queries inteligentes para pesquisa massiva e ultra-detalhada"""

        # Base expandida com muito mais profundidade
        queries_mercado = [
            f"mercado {segmento} Brasil 2024 dados estatísticas crescimento tamanho",
            f"análise mercado {segmento} projeções futuro tendências Brasil",
            f"segmentação mercado {segmento} nichos oportunidades Brasil",
            f"pesquisa consumidor {segmento} comportamento hábitos Brasil 2024",
            f"dados demográficos {segmento} público-alvo perfil consumidor"
        ]

        queries_competitivas = [
            f"análise competitiva {segmento} principais empresas líderes",
            f"concorrentes {segmento} market share participação mercado",
            f"benchmarking {segmento} melhores práticas estratégias",
            f"posicionamento competitivo {segmento} vantagens diferenciais",
            f"matriz competitiva {segmento} forças fraquezas oportunidades"
        ]

        queries_tendencias = [
            f"tendências {segmento} inovações tecnológicas 2024 2025",
            f"futuro {segmento} transformação digital disruption",
            f"oportunidades investimento {segmento} venture capital Brasil",
            f"emerging trends {segmento} new technologies innovations",
            f"megatrends {segmento} impacto mercado brasileiro"
        ]

        queries_startups = [
            f"startups {segmento} investimento funding rodadas Brasil",
            f"ecossistema empreendedorismo {segmento} aceleradoras",
            f"venture capital {segmento} investimentos Brasil 2024",
            f"unicórnios brasileiros {segmento} cases sucesso",
            f"inovação {segmento} startups disruptivas Brasil"
        ]

        queries_casos = [
            f"cases sucesso empresas {segmento} brasileiras estratégias",
            f"histórias sucesso {segmento} lições aprendidas",
            f"empresas líderes {segmento} modelos negócio inovadores",
            f"transformação digital {segmento} casos práticos Brasil",
            f"melhores práticas {segmento} benchmarking internacional"
        ]

        queries_desafios = [
            f"desafios principais {segmento} soluções inovadoras mercado",
            f"barreiras entrada {segmento} como superar obstáculos",
            f"problemas comuns {segmento} estratégias resolução",
            f"gargalos {segmento} oportunidades melhoria",
            f"pain points consumidores {segmento} necessidades não atendidas"
        ]

        queries_especificas = []
        if produto:
            queries_especificas = [
                f"{produto} {segmento} análise mercado brasileiro",
                f"demanda {produto} {segmento} potencial crescimento",
                f"concorrentes {produto} {segmento} comparativo",
                f"preços {produto} {segmento} estratégias pricing",
                f"canais distribuição {produto} {segmento} Brasil"
            ]

        # Combina todas as queries
        all_queries = (queries_mercado + queries_competitivas + queries_tendencias +
                      queries_startups + queries_casos + queries_desafios + queries_especificas)

        return all_queries[:15]  # Expandido para 15 queries mais robustas


    def _format_time(self, seconds: float) -> str:
        """Formata tempo em segundos para minutos e segundos"""
